...
case class Procedure(op: Symbol, args: SList, params: SList, f: Environment => Expression => Expression) extends Value {
    override def printAST: String = "Proc(" + op.symbol + ", " + params.printAST + ", " + f + ")"

    override def print: String = op.symbol + " " + params

    override def toString: String = "(" + print + ")"
}

object Procedure {
    def apply(op: Symbol, params: SList, f: Environment => Expression => Expression): Procedure =
        new Procedure(op, Empty, params, f)
}
