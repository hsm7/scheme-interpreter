...
object Environment {
  ...
  implicit lazy val global: Environment = {
    ...
    Environment(mutable.Map[Symbol, Expression](
      Symbol("<") -> Lambda(params, _ => Predef.lt),
      Symbol("+") -> Lambda(params, _ => Predef.add),
      Symbol("-") -> Lambda(params, _ => Predef.subtract),
      Symbol("*") -> Lambda(params, _ => Predef.multiply),
      Symbol("/") -> Lambda(params, _ => Predef.divide),
      Symbol("equal?") -> Lambda(params, _ => Predef.equal),
      Symbol("car") -> Lambda(param, _ => Predef.car),
      Symbol("cdr") -> Lambda(param, _ => Predef.cdr),
      Symbol("empty?") -> Lambda(param, _ => Predef.empty),
      Symbol("cons") -> Lambda(params, _ => Predef.cons),
